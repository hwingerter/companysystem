...
if ($credencial_ver == '1') { //VERIFICA SE USU�RIO POSSUI ACESSO A ESSA �REA
	...
	if ($excluir != '') {
		$sql = "delete from clientes where cod_cliente = ". $excluir;
		mysql_query($sql);
	...
	$sql = "Select COUNT(cod_cliente) as total from clientes 
	where cod_empresa = ".$cod_empresa."  ";
	
	if (isset($_REQUEST['acao'])){
		if ($_REQUEST['acao'] == "buscar"){
			if ($_REQUEST['nome'] != ""){
				$sql = $sql . " where nome like '%".$_REQUEST['nome']."%' or empresa like '%".$_REQUEST['nome']."%' ";
			}
		}
	}	
...
<li class="active"><a href="clientes.php">Clientes</a></li>
                            ...
                            <div class="page-heading">            
							  ...
							  ?>
								<a class="btn btn-midnightblue btn-label" href="cliente_info.php"><i class="fa fa-plus-circle"></i> Novo</a>
							  <?php
							  }
							  ?>	
								</div>
...
<script language="JavaScript">
	function paginacao () {
  		document.forms[0].action = "clientes.php";
...
<form name="form1" method="post">
				...
				<div class="alert alert-dismissable alert-info">
					...
					<br><a class="btn btn-success" href="clientes.php?excluir=<?php echo $pergunta;?>">Sim</a>&nbsp;&nbsp;&nbsp; <a class="btn btn-danger" href="clientes.php">Não</a>
				</div>				
				<?php
				}
...
<div class="row">
	...
	if (isset($_REQUEST['acao'])){
		if ($_REQUEST['acao'] == "buscar"){
			if ($_REQUEST['nome'] != ""){
				$sql = $sql . " and c.nome like '%".$_REQUEST['nome']."%' ";
			}
		}
	}
	...
	if ($registros > 0) {
		...
		?>
                        <tr>
                            <td align="left"><?php echo $rs['cod_cliente'];?></td>
							<td align="left"><?php echo $rs['nome'];?>
							</td>
						  ...
						  ?>							
							<a class="btn btn-success btn-label" href="cliente_info.php?acao=alterar&id=<?php echo $rs['cod_cliente'];?>"><i class="fa fa-edit"></i> Editar</a>&nbsp;
					  	  <?php 
						  }
						  if ($credencial_excluir == '1') {
						  ?>
							<a class="btn btn-danger btn-label" href="clientes.php?pergunta=<?php echo $rs['cod_cliente'];?>"><i class="fa fa-times-circle"></i> Excluir</a>
						  <?php
						  }
						  ?>	
							<a class="btn btn-info btn-label" href="cliente_ver.php?id=<?php echo $rs['cod_cliente'];?>"><i class="fa fa-eye"></i> Ver</a>
							</td>
                        </tr>
    <?php
			} // Contador
