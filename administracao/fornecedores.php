...
if ($_SESSION['usuario_conta'] == '1') { //VERIFICA SE USU¡RIO POSSUI ACESSO A ESSA ¡REA
	...
	if ($excluir != '') {
		$sql = "delete from fornecedores_company where cod_fornecedor = ". $excluir;
		mysql_query($sql);
	...
	$sql = "Select COUNT(cod_fornecedor) as total from fornecedores_company  ";
	...
	if ($resto > 0) { $paginas = $paginas + 1; } // Se o resto maior do que 0, soma a var paginas para a paginaÁ„o ficar correta
...
<li class="active"><a href="fornecedores.php">Fornecedores</a></li>
                            ...
                            <div class="page-heading">            
							  ...
							  ?>
								<a class="btn btn-midnightblue btn-label" href="fornecedor_info.php"><i class="fa fa-plus-circle"></i> Novo</a>
							  <?php
							  }
							  ?>	
								</div>
...
<script language="JavaScript">
	function paginacao () {
  		document.forms[0].action = "fornecedores.php";
...
<form name="form1" method="post">
				...
				<div class="alert alert-dismissable alert-info">
					<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
					<strong>Deseja realmente excluir o c√≥digo n√∫mero <?php echo $pergunta; ?> ?</strong><br>
					<br><a class="btn btn-success" href="fornecedores.php?excluir=<?php echo $pergunta;?>">Sim</a>&nbsp;&nbsp;&nbsp; <a class="btn btn-danger" href="fornecedores.php">N√£o</a>
				</div>				
				<?php
				}
...
<div class="row">
    ...
    <?php
		//CARREGA LISTA
	$sql = "
	select		e.cod_fornecedor, e.empresa, e.email, e.telefone from fornecedores_company e";
	...
	$sql .= "
	group by	e.cod_fornecedor, e.empresa
	...
	if ($registros > 0) {
		while (($rs = mysql_fetch_array($query)) && ($contador<$final)){ 
	    	...
	    	if ($contador>$inicio) { //CondiÁao para mostrar somente os registros maiores
		?>
                        <tr>
                            <td align="left"><?php echo $rs['cod_fornecedor'];?></td>
							<td align="left">
							<?php echo $rs['empresa']; ?>
							</td>
						  ...
						  ?>							
							<a class="btn btn-success btn-label" href="fornecedor_info.php?acao=alterar&id=<?php echo $rs['cod_fornecedor'];?>"><i class="fa fa-edit"></i> Editar</a>&nbsp;
					  	  <?php 
						  }
						  if ($credencial_excluir == '1') {
						  ?>
							<a class="btn btn-danger btn-label" href="fornecedores.php?pergunta=<?php echo $rs['cod_fornecedor'];?>"><i class="fa fa-times-circle"></i> Excluir</a>
						  <?php
						  }
						  ?>	
							<a class="btn btn-info btn-label" href="fornecedor_ver.php?id=<?php echo $rs['cod_fornecedor'];?>"><i class="fa fa-eye"></i> Ver</a>
							</td>
                        </tr>
    <?php
			} // Contador
...
<?php
	...
	?>
                        <tr>
                            <td align="center" colspan="7">N√£o tem nenhum registro!</td>
                        </tr>
<?php
	}
?>		
