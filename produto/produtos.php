...
if ($credencial_ver == '1') { //VERIFICA SE USU�RIO POSSUI ACESSO A ESSA �REA
	...
	if ($excluir != '') {
		$sql = "delete from produtos where cod_produto = ". $excluir;
		mysql_query($sql);
...
<li class="active"><a href="produtos.php">Produtos</a></li>
                            ...
                            <div class="page-heading">            
							  ...
							  ?>
								<a class="btn btn-midnightblue btn-label" href="produto_info.php"><i class="fa fa-plus-circle"></i> Novo</a>
							  <?php
							  }
							  ?>	
								</div>
...
<form name="form1" method="post">
				...
				<div class="alert alert-dismissable alert-info">
					...
					<br><a class="btn btn-success" href="produtos.php?excluir=<?php echo $pergunta;?>">Sim</a>&nbsp;&nbsp;&nbsp; <a class="btn btn-danger" href="produtos.php">Não</a>
				</div>				
				<?php
				}
...
<div class="row">
                ...
                <table class="table" border="1" bordercolor="#EEEEEE">
                    <thead>
                        <tr>
							...
							<th width="250">Fornecedor</th>
							<th width="100">Preço de Venda</th>
    ...
    <?php
		//CARREGA LISTA
	$sql = "
	select		p.cod_produto, p.descricao, p.preco_venda, f.nome_fantasia as fornecedor
	from 		produtos p
	left join 	fornecedores f on f.cod_fornecedor = p.cod_fornecedor
	left join	empresas e on e.cod_empresa = f.cod_empresa
	where 		p.cod_empresa = ".$cod_empresa." ";

	if (isset($_REQUEST['acao'])){
		if ($_REQUEST['acao'] == "buscar"){
			if ($_REQUEST['nome'] != ""){
				$sql = $sql . " and p.descricao like '%".$_REQUEST['nome']."%' ";
			}
		}
	}
	$sql .= "
	group by	p.cod_produto, p.descricao, p.preco_venda
	...
	if ($registros > 0) {
		...
		?>
                        <tr>
                            <td align="left"><?php echo $rs['cod_produto'];?></td>
							<td align="left">
							<?php echo $rs['descricao']; ?>
							</td>
							<td align="left"><?php echo $rs['fornecedor'];?></td>
							<td align="left"><?php echo ValorMysqlPhp($rs['preco_venda']);?></td>
                            <td align='center'>
					  	  <?php 
						  if ($credencial_editar == '1') {
						  ?>							
							<a class="btn btn-success btn-label" href="produto_info.php?acao=alterar&id=<?php echo $rs['cod_produto'];?>"><i class="fa fa-edit"></i> Editar</a>&nbsp;
					  	  <?php 
						  }
						  if ($credencial_excluir == '1') {
						  ?>
							<a class="btn btn-danger btn-label" href="produtos.php?pergunta=<?php echo $rs['cod_produto'];?>"><i class="fa fa-times-circle"></i> Excluir</a>
						  <?php
						  }
						  ?>	
							<a class="btn btn-info btn-label" href="produto_ver.php?id=<?php echo $rs['cod_produto'];?>"><i class="fa fa-eye"></i> Ver</a>
							</td>
                        </tr>
    <?php
			} // Contador
