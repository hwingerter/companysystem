...
if (($credencial_incluir == '1') || ($credencial_editar == '1')) { // Verifica se o usu�rio tem a credencial de incluir ou editar	
	...
	if (isset($_REQUEST['acao'])){
		...
		if($_REQUEST['acao'] == "realizar_pagamento"){
			...
			************************************************/
			if (isset($_REQUEST["qtd_cartao_credito"]) && ($_REQUEST["qtd_cartao_credito"] != "0")) { 
			
				$qtd_cartao_credito = $_REQUEST["qtd_cartao_credito"];

				$soma_parcelas = 0;

				for($i=1; $i<=$qtd_cartao_credito; $i++){

					if (isset($_REQUEST["cod_cartao_credito_".$i])) { $cod_cartao_credito = limpa($_REQUEST["cod_cartao_credito_".$i]); } else { $cod_cartao_credito = "NULL"; }
					if (isset($_REQUEST["num_parcelas_".$i])) { $num_parcelas = limpa($_REQUEST["num_parcelas_".$i]); } else { $num_parcelas = "NULL"; }
					if (isset($_REQUEST["valor_cartao_credito_".$i])) { $valor = ValorPhpMysql($_REQUEST["valor_cartao_credito_".$i]); } else { $valor = "NULL"; }
				
					$soma_parcelas += $valor_cartao_credito;
								...
								<div data-widget-group="group1">
										...
										<div class="panel panel-default" data-widget='{"draggable": "false"}'>
											...
											<div class="panel-body">
												...
												<form action="comanda_pagamento_cartao_credito.php" class="form-horizontal row-border" name='frm' method="post">
												  ...
												  <input type="hidden" name="cod_forma_pagamento" value="<?php echo $cod_forma_pagamento; ?>">
													...
													<div class="form-group">
														...
														<p class="mb20" style="border-bottom:1px solid black;">Cartão de Crédito</p>

														<div class="col-sm-4">
															<label class="col-sm-2 col-sm-offset-1 control-label"><b>Cartões</b></label>
															<div class="col-sm-4 col-sm-offset-2">
															<select name='qtd_cartao_credito' class="form-control col-sm-offset-1" style="width: 100px;" onChange="javascript:AbrirPagamentoCartaoCredito(this.value);">
															  	<?php for ($i=1; $i<=12; $i++) {?>
													...
													for ($i=1; $i<=12; $i++) { 
														...
														<div id="cartao_credito_<?php echo $i;?>" <?php echo $ocultar; ?> >

															<div class="form-group">
																<label class="col-md-2 control-label"><b><?php echo $i; ?>a Cartao</b></label>
															</div>

															<div class="form-group">
																<label class="col-sm-2 control-label"><b>Bandeira</b></label>
																<div class="col-sm-8">
																	<?php comboCartaoCredito($cod_empresa, $i, ''); ?>
																</div>
															</div>
															...
															<div class="form-group">
																...
																<div class="col-sm-8">
																	<select name='num_parcelas_<?php echo $i;?>' class="form-control" style="width: 100px;">
																	<?php for ($j=1; $j<=12; $j++) {?>
																	<option value="<?php echo $j; ?>" <?php if ($parcelas == $j) { echo " Selected"; }?>><?php echo $j;?>x</option>
																	<?php
																			}
																	?>
																	</select>
															...
															<div class="form-group">
																...
																<div class="col-sm-8">
																	<input type="text" class="form-control" value="" name="valor_cartao_credito_<?php echo $i;?>" id="valor_cartao_credito_<?php echo $i;?>" maxlength="100"
