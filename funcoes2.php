...
//define("host", "http://localhost/belezasoft/");
...
function ComboLicenca($cod_licenca) 
{
	...
	$sql = "
	select cod_licenca, descricao from licencas order by 1;
	";
	$query = mysql_query($sql);
	$registros = mysql_num_rows($query);
	if ($registros > 0) {
		...
		while ($rs = mysql_fetch_array($query)){ 
			...
			echo $retVal.">".$rs['descricao'] . "</option>";	
...
function ComboLicencaRenovacao() {
	...
	if ($registros > 0) {
		...
		while ($rs = mysql_fetch_array($query)){ 
			echo "<option value='". $rs['cod_licenca'] ."'";
			echo ">".$rs['descricao']." (".$rs['valor'].")". "</option>";	
...
function AdmComboTipoConta($tipo) {
	
	$sql = "
	select		tc.cod_tipo_conta, g.nome as grupo, tc.descricao as tipoconta
	from 		tipo_conta tc
	left join	grupos g on g.cod_grupo = tc.cod_grupo
	...
	if ($registros > 0) {
		...
		while ($rs = mysql_fetch_array($query)){ 
			...
			if ($tipo == $rs['cod_tipo_conta']) { echo " Selected"; }

			if ($rs['cod_tipo_conta']==1){
				echo ">".$rs['tipoconta'] . "</option>";	
			}else{
				echo ">(".$rs['grupo'].") - ".$rs['tipoconta'] . "</option>";
			}
...
function ComboTipoConta($tipo, $grupo) {
	
	$sql = "Select * from tipo_conta where cod_tipo_conta <> 1 ";

	if ($grupo != ""){
		$sql=$sql."and cod_grupo = ".$_SESSION['cod_grupo']." ";
	}
...
function ComboTipoComissao($cod_tipo_comissao) {
	...
	$registros = mysql_num_rows($query);
	if ($registros > 0) {
		echo "<select name='cod_tipo_comissao' class='form-control' id='cod_tipo_comissao' onChange='Javascript:MudarTipoComissao(this.value);'>";
		while ($rs = mysql_fetch_array($query)){ 
...
function TotalComanda($cod_empresa, $cod_cliente, $cod_comanda){
	...
	$sql = "
	select 		format(sum(ifnull( ((i.valor * i.quantidade)) , '0.00')), 2) as total_comanda 
	from 		comanda_item i 
	inner join 	comanda c on c.cod_comanda = i.cod_comanda
	where 		i.cod_comanda in (".$cod_comanda.")
	...
	return number_format($rs['total_comanda'], 2, ',', '.');
...
function TotalPago($cod_empresa, $cod_cliente, $cod_comanda){
	...
	$sql = "
	select 		ifnull(sum(valor*(ifnull(num_parcelas, 1))), 0.00) as valor_pago
	...
	$query = mysql_query($sql);
	$rs = mysql_fetch_array($query);
	return number_format($rs['valor_pago'], 2, ',', '.');
...
function Comanda_Desconto($cod_empresa, $cod_cliente, $cod_comanda){
	...
	from 		comanda
	where 		cod_comanda in (".$cod_comanda.")
	and 		cod_cliente = ".$cod_cliente." 
...
function Comanda_AtualizaSituacao($cod_empresa, $cod_cliente, $cod_comanda, $cod_forma_pagamento){
	...
	//echo $total_comanda." - ".$total_pago." - ".$cod_forma_pagamento; die;

	if($cod_forma_pagamento == "1" || $cod_forma_pagamento == "2" || $cod_forma_pagamento == "3" || $cod_forma_pagamento == "4" || $cod_forma_pagamento == "5" || $cod_forma_pagamento == "6"){
...
function ComboFormaPagamento() 
{
	...
	<select name="cod_forma_pagamento" id="cod_forma_pagamento" class="form-control" onChange="SelecionarFormaPagamento(this.value);">
...
function ComboBanco($cod_empresa, $i, $cod_banco) 
{
	...
	<select name="cod_banco_<?php echo $i; ?>" id="cod_banco_<?php echo $i; ?>" class="form-control">
		...
		?>
			...
			<option value="<?php echo $rs['cod_banco']; ?>" <?php if ($cod_banco == $rs['cod_banco']){echo " selected "; } ?>> <?php echo $rs['descricao']; ?> </option>
