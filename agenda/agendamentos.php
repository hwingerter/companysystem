...
?>
		...
		<script language="javascript" src="js/comanda.js"></script>
			...
			<form action="agendamentos.php" class="form-horizontal" name='frm' method="post">
						...
						<div class="panel panel-sky">
							...
							<div class="panel-body">
								...
								<div class="form-group">
									...
									<div class="col-sm-6">
										<?php comboFuncionario($cod_empresa, $cod_profissional); ?>
									</div>
								</div>								
          ...
          <div class="panel-body">
            ...
            <div class="table-responsive">
				...
				<?php
						...
						//CARREGA LISTA
					$sql = "
					select		p.nome as profissional, date_format(a.dt_agenda, '%d/%m/%Y') as data, a.hora, c.nome as cliente
								,c.telefone, s.nome as servico, a.obs
					from 		agenda a
					inner join 	clientes c on c.cod_cliente = a.cod_cliente
					inner join 	profissional p on p.cod_profissional = a.cod_profissional
					inner join 	servico s on s.cod_servico = a.cod_servico
					...
					if (isset($_REQUEST['acao'])){
						...
						if ($_REQUEST['acao'] == "buscar"){
							...
							if ($_REQUEST['cod_cliente'] != ""){
								$sql = $sql . " and c.cod_cliente = ".$_REQUEST['cod_cliente']." ";
							}

							if ($_REQUEST['cod_profissional'] != ""){
								$sql = $sql . " and p.cod_profissional = ".$_REQUEST['cod_profissional']." ";
							}

							if (isset($_REQUEST['dt_inicial']) && ($_REQUEST['dt_inicial'] != ""))
							{
								$sql = $sql . " and a.dt_agenda >= '".DataPhpMysql($_REQUEST['dt_inicial'])." 00:00:00'";
							}

							if (isset($_REQUEST['dt_final']) && ($_REQUEST['dt_final'] != ""))
							{
								$sql = $sql . " and a.dt_agenda <= '".DataPhpMysql($_REQUEST['dt_final'])." 23:59:59'";
							}
                ...
                <table class="table" border="1" bordercolor="#EEEEEE">
					...
					if ($registros > 0) {
                        ...
                        <tr>
                            ...
                            <td align="left"><?php echo $rs['servico'];?></td>
